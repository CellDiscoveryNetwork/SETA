...
#' from a given source object (Seurat, SingleCellExperiment,
#' or dataframe). It ensures that each metadata column contains
...
#'
#' @param source_obj An object of class Seurat, SingleCellExperiment,
#' or a dataframe which contains cell-level or sample-level metadata.
...
#' }
#' @export
setaMetadata <- function(source_obj,
  ...
  if (inherits(source_obj, "Seurat")) {
    ...
    stop("Unsupported object type. 
          Must be Seurat, SingleCellExperiment, or dataframe.")
  }
  ...
  if (length(nu_cols) > 1) {
    ...
    summarized_meta <- summarized_meta[,
            !(colnames(summarized_meta) %in% nu_cols[nu_cols != sample_col])]
  }
