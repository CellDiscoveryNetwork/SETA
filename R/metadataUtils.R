...
setaMetadata <- function(x, sample_col = "Sample ID", meta_cols) {
    ...
    if (is.factor(x[[sample_col]])) {
        out$sample_id <- factor(out$sample_id, levels = levels(x[[sample_col]]))
    }
    for (col in meta_cols) {
        col_data <- x[[col]]
        vals <- vapply(samples, function(sid) {
            u <- unique(col_data[x[[sample_col]] == sid])
            if (length(u) != 1L) stop(
                sprintf(
            ...
            u
        }, FUN.VALUE = col_data[1])
        out[[col]] <- if (is.factor(col_data)) {
            factor(vals, levels = levels(col_data))
