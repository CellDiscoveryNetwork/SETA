...
#' this function produces a type-by-sample matrix of cell counts. Users can specify
#' the names of the columns for cell types, samples, and barcodes. The function
#' ensures that each cell is counted once per type-sample combination by using
#' unique barcodes.
#'
...
#' @details
#' \itemize{
#'   \item \strong{SingleCellExperiment}: Reads \code{colData(obj)} for the specified
#'   \code{cell_type_col} and \code{sample_col}.
#'   \item \strong{Seurat}: Uses \code{obj@meta.data} for the specified
#'   \code{cell_type_col} and \code{sample_col}.
#'   \item \strong{data.frame}: The function first removes duplicates based on the \code{bc_col}
#'   to ensure each cell is counted once, then counts cells per specified \code{cell_type_col}
#'   and \code{sample_col}.
#' }
...
#'
#' @export
setaCounts <- function(obj, cell_type_col = "type", sample_col = "sample", bc_col = "bc") {
  ...
  if ("SingleCellExperiment" %in% class(obj)) {
    ...
    if (!all(c(cell_type_col, sample_col) %in% colnames(coldata))) {
      stop(sprintf("colData(obj) must contain '%s' and '%s' columns.", cell_type_col, sample_col), call. = FALSE)
    }
  ...
  if ("Seurat" %in% class(obj)) {
    ...
    if (!all(c(cell_type_col, sample_col) %in% colnames(meta))) {
      stop(sprintf("obj@meta.data must contain '%s' and '%s' columns.", cell_type_col, sample_col), call. = FALSE)
    }
  ...
  if (is.data.frame(obj)) {
    ...
    if (!all(requiredCols %in% colnames(obj))) {
      stop(sprintf("data.frame must have columns: '%s', '%s', '%s' at minimum.", bc_col, cell_type_col, sample_col), call. = FALSE)
    }
